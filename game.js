/**
 * Created by Robert on 05/07/14.
 */

var appleSound = new buzz.sound("sounds/apple", {
    formats: [ "mp3"]
});

var gameOverSound = new buzz.sound("sounds/game_over", {
    formats: [ "mp3"]
});
...
//            timeout = setTimeout(gameLoop, frameLength);
    ...
    function gameOver(){
        ...
        $('#submit-score').on('click', function (e){
            ...
            if (name == ''){
                ...
                $.ajax({
                    ...
                    $('#score-form').html('<p>Your score has been saved!</p>');
                    $('#scores').find('tbody').html(result);
                });
            }
        });
        ...
        $('#game-over p').on('click', function (e){
            e.stopPropagation();
        });
    ...
    function restart(){
        ...
        $(canvas).unbind('click');
        $('#game-over').hide();
        $('#score-form').html('<p>Enter your name to save your score</p><p><input type="text" name="name" id="name"/></p><p><button id="submit-score">Submit</button></p>');
...
JS_SNAKE.apple = function (){
    ...
    function draw(ctx){
        ctx.save();
        ctx.fillStyle = '#4FB99F'; //apple green
        ctx.beginPath();
